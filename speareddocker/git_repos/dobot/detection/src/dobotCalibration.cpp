 ...
 *
 *	Used to search the transform between world and dobot. It communicates with the tfPublisher, which is responsible for all static coordinate frames.
...
//Remoes all points from the viewer
void removePoints()
{
...
//Draws a point with id at the given position into the 3d viewer
...
//Draws all points into the 3D viewer
void drawPoints()
{
	removePoints();
...
//Based on the dobotPoints and the world points, it estimates a transform between them, which is the pose of the dobot
void minimizeWithEigen()
{
	...
	Eigen::Matrix<double,4,4> solution = Eigen::umeyama(start,end); //this does the pose estimation
...
//Called when the median filter is done and a new pointcloud is ready for the viewer
...
//Resturns the closest point from a set of 3D points
...
void commandDobotArm(tf2::Vector3 goal)
{
	...
	if(arm.hasLinearRail())
	{
		...
		{
			arm.moveArmToPositionWithL(graspPoint + tf2::Vector3(xOffset,0,zOffset),0,railPos+yOffset);
		}
	}else{
		...
		{
			arm.moveArmToPosition(graspPoint+tf2::Vector3(xOffset,yOffset,zOffset),0);
		}
	}
}
...
void mouseClickEvent(const pcl::visualization::PointPickingEvent &event, void*)
{
	...
	if(clickMoveMode)//Give direct commands to the dobot?
	{
...
int main (int argc, char* argv[])
{
	...
	//Does the coordinate frame exist? Get it as an initial value
	try{
		geometry_msgs::TransformStamped tS;
	...
	while (ros::ok())
	{
		...
		//Update the robot arm position in the viewer
		geometry_msgs::TransformStamped tS;
