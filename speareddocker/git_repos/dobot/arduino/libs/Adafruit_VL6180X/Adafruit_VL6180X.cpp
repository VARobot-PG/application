...
*/
/**************************************************************************/
boolean Adafruit_VL6180X::begin(TwoWire *theWire, uint8_t i2c_addr) {
  _i2caddr = i2c_addr;
  if (! theWire) {
    _i2c = &Wire;
  } else {
...
uint8_t Adafruit_VL6180X::sample_available()
{
	return read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01;
}

uint8_t Adafruit_VL6180X::readRangeNonBlocking(void) {
...
void Adafruit_VL6180X::override_i2c_address(uint8_t address)
{
	write8(VL6180X_REG_I2C_SLAVE_DEV_ADDR, address);
	_i2caddr = address;
}

void Adafruit_VL6180X::enable_snr_check()
{
	write8(VL6180X_SYSRANGE_RANGE_CHECK_ENABLE, 
				read8(VL6180X_SYSRANGE_RANGE_CHECK_ENABLE) | 8);
}
