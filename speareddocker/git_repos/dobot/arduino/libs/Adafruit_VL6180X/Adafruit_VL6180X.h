...
#define VL6180X_DEFAULT_I2C_ADDR 0x29  ///< The default I2C address
...
#define VL6180X_SYSRANGE_RANGE_CHECK_ENABLE    0x02D
///! Trigger Lux Reading
...
#define VL6180X_REG_I2C_SLAVE_DEV_ADDR				 0x212
...
class Adafruit_VL6180X {
 public:
  Adafruit_VL6180X();
  boolean begin(TwoWire *theWire = NULL, uint8_t i2c_address = VL6180X_DEFAULT_I2C_ADDR);
  uint8_t readRange(void);
  float   readLux(uint8_t gain);
  uint8_t readRangeStatus(void);
	uint8_t readRangeNonBlocking(void);
	uint8_t sample_available();
	void override_i2c_address(uint8_t address);
	void enable_snr_check();
 private:
