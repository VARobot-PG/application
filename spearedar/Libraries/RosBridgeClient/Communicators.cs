        ...
        public static string GetRosName<T>() where T : Message
        {
            ...
            try
            {
                ...
                if(messageType != null){
                     ...
                     FieldInfo  fi = messageType.GetField("RosMessageName");
                     if(fi != null){
                         Object value = fi.GetValue(null);
                            if(value != null){
                                rosname = (string) value;
                            }else{
                                rosname = "type_"+ ti.FullName + "_valueNull_getROSName";
                         }
                    }
                    else{
                        rosname = "type_"+ ti.FullName + "_fieldInfoNull_getROSName";
                        if(rosname.Contains("RosSharp.RosBridgeClient.MessageTypes.Nav.Odometry")){
                            rosname = "nav_msgs/Odometry";
                        }
                        if(rosname.Contains("RosSharp.RosBridgeClient.MessageTypes.Std.Bool")){
                            rosname = "std_msgs/Bool";
                        }
                    }
                }else{
                    rosname = "typeInfoNull_getROSName";
                }
            //rosname = (string).GetDeclaredField("RosMessageName").GetValue(null);
                       
#else
                rosname = (string)typeof(T).GetField("RosMessageName").GetRawConstantValue();

#endif
            }
            catch (NullReferenceException nre)
            {
                rosname = "NullReferenceException getROSName";
            }
            return rosname;
        }
    }
